...
export function AddStaffForm({
  ...
  useEffect(() => {
    if (formData.fullName.trim()) {
      setErrors((prev) => {
        if (prev.fullName) {
          console.log("[AddStaffForm] Clearing fullName error");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    console.log("[AddStaffForm] Form submitted", formData);
    ...
    if (!validateForm(formData)) {
      console.log("[AddStaffForm] Validation failed");
      return;
    }

    setIsSubmitting(true);
    console.log("[AddStaffForm] Submitting...");

    try {
      await onSubmit(formData);
      console.log("[AddStaffForm] Submit success");
    } catch (error) {
  ...
  const handleButtonClick = () => {
    console.log("[AddStaffForm] Create Staff button clicked");
  };
